...
pub const GPA_SCALE: u64 = 400; // GPA scale (4.0 * 100)
pub const MAX_GPA_BONUS: i128 = 200; // 20% bonus at 4.0 GPA
pub const MIN_GPA_THRESHOLD: u64 = 200; // 2.0 minimum GPA for scholarship
pub const FLOW_RATE_PRECISION: u64 = 1000; // Precision for flow rate calculations
...
pub enum DataKey {
    ...
    StudyGroup(u64), // group_id -> StudyGroup
    MemberCollateral(Address), // member -> MemberCollateral
    VoteRecord(Address, Address, u64), // (voter, target_member, group_id) -> VoteRecord
    NextGroupId, // Counter for group IDs
    GroupVoteCount(u64, Address, Symbol), // (group_id, target_member, vote_type) -> vote count
    // GPA and math verification keys
    GpaRecord(Address), // student -> GpaRecord
    FlowRateAdjustment(Address), // student -> FlowRateAdjustment
    BudgetTracker(Address), // student -> BudgetTracker
    AcademicOracle(Address), // verifier address -> authorized status
}
