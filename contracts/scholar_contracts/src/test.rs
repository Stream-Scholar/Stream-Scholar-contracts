...
impl MockDexContract {
    pub fn path_payment_strict_receive(
        ...
        let received_amount = send_amount;
        if received_amount < min_received {
            env.panic_with_error((soroban_sdk::xdr::ScErrorType::Contract, soroban_sdk::xdr::ScErrorCode::InvalidAction));
        }

        let dest_client = token::Client::new(&env, &destination_token);
        dest_client.transfer(&env.current_contract_address(), &destination, &received_amount);
        received_amount
    }
}
